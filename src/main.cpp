...
History * run_tests(bool print, double maxtime, int num_stages, double * par, double dispersal, double * interactions,
...
// [[Rcpp::export]]
Rcpp::DataFrame test_parameter(double maxtime,int num_stages,Rcpp::NumericVector parameters, double dispersal, 
		Rcpp::NumericVector interactions,
		Rcpp::IntegerVector init, double w=100, double h=100, int bcond=1, int dkernel=1, int maxpop=30000){
	int *in;
	History * ret;
	in = init.begin();

	ret = run_tests(false,maxtime, num_stages,parameters.begin(),dispersal,interactions.begin(),
			w,h,in,bcond,dkernel,maxpop);
